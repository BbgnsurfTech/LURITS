                    ...
                    <tbody>
                        @foreach($teams as $key => $team)
                            <tr data-entry-id="{{ $team->id }}">
                                ...
                                <td>
                                    @can('team_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.schools.show', $team->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('team_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.schools.edit', $team->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('team_delete')
                                        <form action="{{ route('admin.schools.destroy', $team->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
