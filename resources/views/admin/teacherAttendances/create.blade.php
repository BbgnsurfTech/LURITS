...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.teacherAttendance.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.teacher-attendances.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('admission') ? 'is-invalid' : '' }}" name="admission_id" id="admission_id">
                    @foreach($admissions as $id => $admission)
                        <option value="{{ $id }}" {{ old('admission_id') == $id ? 'selected' : '' }}>{{ $admission }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\TeacherAttendance::ATTENDANCE_STATUS_MORNINIG_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('attendance_status_morninig') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="attendance_status_morninig_{{ $key }}" name="attendance_status_morninig" value="{{ $key }}" {{ old('attendance_status_morninig', '255') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\TeacherAttendance::ATTENDANCE_STATUS_AFTERNOON_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('attendance_status_afternoon') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="attendance_status_afternoon_{{ $key }}" name="attendance_status_afternoon" value="{{ $key }}" {{ old('attendance_status_afternoon', '255') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\TeacherAttendance::LATE_STATUS_Y_N_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('late_status_y_n') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="late_status_y_n_{{ $key }}" name="late_status_y_n" value="{{ $key }}" {{ old('late_status_y_n', '255') === (string) $key ? 'checked' : '' }}>
