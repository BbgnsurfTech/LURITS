                ...
                <select class="form-control select2 {{ $errors->has('assigned_to') ? 'is-invalid' : '' }}" name="assigned_to_id" id="assigned_to_id">
                  <option value="" disabled selected>Please Select</option>
                    @foreach($assigned_tos as $assigned_to)
                        <option value="{{ $assigned_to->id }}" {{ old('assigned_to_id') == $assigned_to->id ? 'selected' : '' }}>{{ $assigned_to->first_name }} {{ $assigned_to->middle_name }} {{ $assigned_to->last_name }} - {{ $assigned_to->staff_id }}</option>
                    @endforeach
                </select>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
