...
class TransferApiController extends Controller
{
    ...
    {
        // $data = StaffTransfer::with(["student", "school_enrolled", "school", "classroom"])->get();
        // return response(
        //     json_decode($data),
        // 200);

        $data = StaffTransfer::where('current_school', Auth::User()->school_id)->orWhere('target_school', Auth::User()->school_id)->get();
    ...
    public function store(StoreStudentAdmissionRequest $request)
    {
        ...
        try {
            ...
            foreach ($allData as $modelData) {
                ...
                if ($modelData['id'] == '' || $modelData['id'] == null) {
                    $data = new StaffTransfer();
                } else {
                    $data = StaffTransfer::find($modelData['id']);
                }
        ...
        if ($final) {
            $message = StaffTransfer::where('school_id', $school_id)->get();
